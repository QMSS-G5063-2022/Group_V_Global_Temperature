...
```{r}
Co2<-read_csv("E:/QMSS/DV/Group_V_Global_Temperature/co2_emission.csv")
```
...
```{r}
htmlwidgets::saveWidget(USA_1, file="USA_1.html")
```
...
```
```{r}
htmlwidgets::saveWidget(Singapore_1, file="Singapore_1.html")
```
...
Year_1<-ggplot()+
  ...
  geom_line(data = By_Year_USA,aes(x= year,y=log10(Emission),color= "log CO2"))+
      xlab("Dates")+
    ylab("Tempt")+
   labs(color="Tempt_Type")+
...
```
```{r}
htmlwidgets::saveWidget(USA_2, file="USA_2.html")
```
...
Year_2<-ggplot()+
  ...
  geom_line(data = By_Year_Russia,aes(x= year,y=log10(Emission),color= "log CO2"))+
      xlab("Dates")+
    ylab("Tempt")+
   labs(color="Tempt_Type")+
...
```
```{r}
htmlwidgets::saveWidget(Russia_2, file="Russia_2.html")
```
```{r}
...
Year_3<-ggplot()+
  ...
  geom_line(data = By_Year_China,aes(x= year,y=log10(Emission),color= "log CO2"))+
      xlab("Dates")+
    ylab("Tempt")+
   labs(color="Tempt_Type")+
...
```
```{r}
htmlwidgets::saveWidget(CN_1, file="CN_1.html")
```
...
By_year_new <-By_Year%>%
  ...
  scale_fill_viridis(option = "B",
                     name= "Temperature Differences by Year",
